...
const PostStock = async (req, res) => {
    try {
        ...
        if(query[0].idItem === request.data.idItem) return new ErrorModel().newNotFound(`Stock del articulo ${request.data.idItem} ya se encuentra en existencia en el sistema.`).send(res);
